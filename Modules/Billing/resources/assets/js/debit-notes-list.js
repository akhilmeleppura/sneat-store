...
document.addEventListener('DOMContentLoaded', function () {
  const dt_debit_note_table = document.querySelector('.debit-note-list-table');
  if (dt_debit_note_table) {
    const dt_debit_note = new DataTable(dt_debit_note_table, {
      ajax: baseUrl + 'accounting/billings/debit-notes/list',
      columns: [
        { data: 'debit_note_id' },
        ...
        },
        {
          ...
          render: function (data, type, full) {
            return (
              ...
              `<a href="javascript:;" data-bs-toggle="tooltip" class="btn btn-icon delete-record" data-id="${full['debit_note_id']}" data-bs-placement="top" title="Delete"><i class="icon-base bx bx-trash icon-md"></i></a>` +
              `<a href="${baseUrl}accounting/billings/debit-notes/${full['debit_note_id']}" data-bs-toggle="tooltip" class="btn btn-icon" data-bs-placement="top" title="Preview Debit Note"><i class="icon-base bx bx-show icon-md"></i></a>` +
              '<div class="dropdown">' +
              ...
              `<a href="${baseUrl}accounting/billings/debit-notes/${full['debit_note_id']}/edit" class="dropdown-item">Edit</a>` +
          ...
          features: [
            {
              ...
              buttons: [
                {
                  text: '<i class="icon-base icon-16px bx bx-plus me-md-2"></i><span class="d-md-inline-block d-none">Create Debit Note</span>',
                  className: 'btn btn-primary',
                  action: function () {
                    window.location = baseUrl + 'accounting/billings/debit-notes/create';
                  }
                }
              ]
            }
      ...
      initComplete: function () {
        let debitNoteStatusContainer = document.querySelector('.debit_note_status');
        if (!debitNoteStatusContainer) {
          debitNoteStatusContainer = document.createElement('div');
          debitNoteStatusContainer.className = 'debit_note_status';
          const filterArea = document.querySelector('.dt-layout-end');
          if (filterArea) {
            filterArea.appendChild(debitNoteStatusContainer);
          }
        }
        this.api()
          .columns(8)
          .every(function () {
            ...
            select.innerHTML = '<option value=""> Debit Note Status </option>';
            debitNoteStatusContainer.appendChild(select);
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          fetch(`${baseUrl}accounting/billings/debit-notes/${debitNoteId}`, {
            ...
            .then(res => res.json())
            .then(data => {
              Swal.fire('Deleted!', data.message || 'Debit note has been deleted.', 'success');
              dt_debit_note.ajax.reload();
              if (modal) {
    ...
    function bindDeleteEvent() {
      const debitNoteTable = document.querySelector('.debit-note-list-table');
      const modal = document.querySelector('.dtr-bs-modal');
      if (debitNoteTable && debitNoteTable.classList.contains('collapsed')) {
        if (modal) {
          modal.addEventListener('click', function (event) {
            if (event.target.closest('.delete-record')) {
              const btn = event.target.closest('.delete-record');
              const debitNoteId = btn.getAttribute('data-id');
              const tooltipInstance = bootstrap.Tooltip.getInstance(btn);
              if (tooltipInstance) tooltipInstance.dispose();
              handleDelete(debitNoteId, null, modal);
            }
          });
        }
      } else {
        const tableBody = debitNoteTable?.querySelector('tbody');
        if (tableBody) {
    ...
    dt_debit_note.on('draw', function () {
