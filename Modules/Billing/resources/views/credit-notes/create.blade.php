...
@endsection
@section('page-script')
    {{-- Removed the generic app-invoice-add.js to avoid conflicts --}}
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            // =========================================================================================
            //  SELECT2 & REPEATER INITIALIZATION
            // =========================================================================================
            $(document).ready(function() {
                ...
                if (invoiceSelect.length) {
                    ...
                    invoiceSelect.on('select2:clear', function(e) {
                ...
                function clearCustomerDetails() {
                    document.getElementById('customerCompany').textContent =
        ...
        function saveCreditNote() {
            ...
            const saveButton = document.getElementById('saveCreditNoteBtn');
            ...
            document.querySelectorAll('.repeater-wrapper').forEach((row, index) => {
                ...
                if (itemId) {
                    ...
                    formData.append(`items[${index}][quantity]`, parseFloat(row.querySelector('.quantity')?.value ||
                        0));
                    ...
                    formData.append(`items[${index}][total_price]`, parseFloat(row.querySelector('.total-price')
                        ?.value || 0));
                    formData.append(`items[${index}][discount_percent]`, parseFloat(row.querySelector('.discount')
                        .textContent) || 0);
                    formData.append(`items[${index}][tax_id]`, row.querySelector('.item-tax-id').value);
                }
            });

            fetch("{{ route('billing.credit-notes.store') }}", {
                    method: 'POST',
                    headers: {
                ...
                .catch(error => {
                    ...
                    Swal.fire({
                        icon: 'error',
                        title: 'Error!',
                        html: `<div>An error occurred while saving the credit note.</div><div class="text-muted small mt-2">${error.message}</div>`,
                        confirmButtonColor: '#3085d6',
                    });
                });
        }
...
@section('content')
    @php
        ...
        $items = $items ?? [];
        $taxes = $taxes ?? [];
        $nextCreditNoteNumber = $nextCreditNoteNumber ?? '#CRN-001';
        $document_prefix = $document_prefix ?? 'CRN-';
    @endphp
    <div class="row invoice-add">
            ...
            <div class="card invoice-preview-card p-sm-12 p-6">
                            ...
                            <dl class="row mb-0 gx-4">
                                ...
                                <dd class="col-sm-7">
                                    <input type="text" class="form-control" disabled
                                        placeholder="{{ $nextCreditNoteNumber ?? '#3905' }}"
                                        value="{{ $nextCreditNoteNumber ?? '#3905' }}" id="creditNoteId"
                ...
                <div class="card-body pt-0 px-0">
                    <form class="source-item" id="creditNoteForm" method="POST"
                        action="{{ route('billing.credit-notes.store') }}">
                        @csrf
                        <!-- Hidden fields -->
                                                        ...
                                                        <div class="col-12">
                                                            ...
                                                            <select name="tax-1-input"
                                                                ...
                                                                @if (count($taxes) > 0)
                                                                    ...
                                                                    @endforeach
                                                                @else
                                                                    <option value="0" data-percentage="0">No tax (0%)
                                                                    </option>
                                                                @endif
                                                            </select>
                                                        </div>
        ...
        <!-- /Credit Note Add-->
        <!-- Credit Note Actions -->
        <div class="col-lg-3 col-12 credit-note-actions">
            <div class="card mb-6">
                <div class="card-body">
                    <button class="btn btn-primary d-grid w-100 mb-4" data-bs-toggle="offcanvas"
                        data-bs-target="#sendInvoiceOffcanvas">
                        <span class="d-flex align-items-center justify-content-center text-nowrap"><i
                                class="icon-base bx bx-paper-plane icon-xs me-2"></i>Send Credit Note</span>
                    </button>
                    <button type="button" class="btn btn-label-secondary d-grid w-100 mb-4"
                        onclick="previewCreditNote()">Preview</button>
                    <button type="button" id="saveCreditNoteBtn" class="btn btn-label-secondary d-grid w-100"
                        onclick="saveCreditNote()">Save</button>
                </div>
            </div>
            <div>
