                    ...
                    <div
                        ...
                        <div class="mb-xl-0 mb-6 text-heading">
                            <div class="d-flex svg-illustration mb-6 gap-2 align-items-center">
                                @if($invoice->items->first() && $invoice->items->first()->branch && $invoice->items->first()->branch->logo)
                                    <img src="{{ asset('storage/branch_logos/' . $invoice->items->first()->branch->logo) }}" alt="Branch Logo" class="app-brand-logo" style="height: 40px;">
                                @else
                            ...
                            @if($invoice->items->first() && $invoice->items->first()->branch)
                ...
                <div class="card-body px-0">
                    <div class="row">
                        <div class="col-xl-6 col-md-12 col-sm-5 col-12 mb-xl-0 mb-md-6 mb-sm-0 mb-6">
                            ...
                            <p class="mb-1">{{ $invoice->customer->city ?? '' }}{{ $invoice->customer->city && $invoice->customer->state ? ', ' : '' }}{{ $invoice->customer->state ?? '' }} {{ $invoice->customer->zip_code ?? '' }}</p>
                            ...
                            <table>
                                <tbody>
                                    <tr>
                                        ...
                                        <td class="fw-medium">${{ number_format($invoice->sub_total - ($invoice->document_discount_amount ?? 0), 2) }}</td>
                                    </tr>
                                    @if($invoice->items->first() && $invoice->items->first()->branch && $invoice->items->first()->branch->bank_name)
                                        <tr>
                                            <td class="pe-4">Bank name:</td>
                                            <td>{{ $invoice->items->first()->branch->bank_name }}</td>
                                        </tr>
                                    @endif
                                    @if($invoice->items->first() && $invoice->items->first()->branch && $invoice->items->first()->branch->bank_country)
                                        <tr>
                                            <td class="pe-4">Country:</td>
                                            <td>{{ $invoice->items->first()->branch->bank_country }}</td>
                                        </tr>
                                    @endif
                                    @if($invoice->items->first() && $invoice->items->first()->branch && $invoice->items->first()->branch->iban)
                                        <tr>
                                            <td class="pe-4">IBAN:</td>
                                            <td>{{ $invoice->items->first()->branch->iban }}</td>
                                        </tr>
                                    @endif
                                    @if($invoice->items->first() && $invoice->items->first()->branch && $invoice->items->first()->branch->swift_code)
                                        <tr>
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td class="text-end px-0 py-6 w-px-100 fw-medium text-heading">
                                    ...
                                    <p class="fw-medium mb-0">${{ number_format($invoice->sub_total - ($invoice->document_discount_amount ?? 0), 2) }}</p>
                                </td>
                            </tr>
                        </tbody>
                    </table>
