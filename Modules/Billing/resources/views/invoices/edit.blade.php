            ...
            $(document).ready(function() {
                ...
                function handleClientSelection(selectElement) {
                    ...
                    if (selected && selected.value && selected.value !== '') {
                        ...
                        document.getElementById('clientCompany').textContent = company || selected
                            .textContent;
                ...
                $('.invoice-form-container [data-repeater-list]').on('DOMNodeInserted', function(e) {
                    ...
                    if ($target.hasClass('repeater-wrapper')) {
                        ...
                        // Reset dropdown inputs to default values
                        const discountInput = $target.find('.item-discount-input')[0];
                        if (discountInput) {
                            discountInput.value = '0';
                        }
                        const taxSelectDropdown = $target.find('.item-tax-select')[0];
                        if (taxSelectDropdown) {
                            taxSelectDropdown.value = '0';
                        }

                        // Reset price displays
                        const sellingPrice = $target.find('.selling-unit-price')[0];
                        if (sellingPrice) {
                            sellingPrice.value = '';
                        }
                        const totalPrice = $target.find('.total-price')[0];
                        if (totalPrice) {
                            totalPrice.value = '0.00';
                        }
                        const discountAmt = $target.find('.item-discount-amount')[0];
                        if (discountAmt) {
                            discountAmt.textContent = '$0.00';
                        }
                        const taxAmt = $target.find('.item-tax-amount')[0];
                        if (taxAmt) {
                ...
                $('.invoice-form-container').on('click', '[data-repeater-create]', function() {
                    setTimeout(function() {
                        // Get the last added row (most recent)
                        ...
                        if ($lastRow.length) {
                            ...
                            $lastRow.find('.discount').text('0%');
                            $lastRow.find('.tax-1').text('0%');
                            $lastRow.find('.item-discount-amount').text('$0.00');
                            $lastRow.find('.item-tax-amount').text('$0.00');
                            ...
                            $lastRow.find('.item-discount-input').val('0');
                            $lastRow.find('.item-tax-select').val('0');
            ...
            document.querySelectorAll('.repeater-wrapper').forEach((row, index) => {
                ...
                if (itemId) {
                    ...
                    if (itemDbId) {
                        ...
                        formData.append(`existing_items[${index}][quantity]`, parseFloat(row.querySelector(
                            '.quantity')?.value || 0));
                        formData.append(`existing_items[${index}][unit_price]`, parseFloat(row.querySelector(
                            '.selling-unit-price')?.value || 0));
                        formData.append(`existing_items[${index}][total_price]`, parseFloat(row.querySelector(
                            '.total-price')?.value || 0));
                        formData.append(`existing_items[${index}][discount_percent]`, parseFloat(row.querySelector(
                            '.discount').textContent) || 0);
                        formData.append(`existing_items[${index}][tax_id]`, row.querySelector('.item-tax-id')
                        .value);
                    } else {
                        formData.append(`items[${index}][item_id]`, itemId);
                        formData.append(`items[${index}][quantity]`, parseFloat(row.querySelector('.quantity')
                            ?.value || 0));
                        formData.append(`items[${index}][unit_price]`, parseFloat(row.querySelector(
                            '.selling-unit-price')?.value || 0));
                        formData.append(`items[${index}][total_price]`, parseFloat(row.querySelector('.total-price')
                            ?.value || 0));
            ...
            fetch("{{ route('billing.invoices.update', $invoice->id) }}", {
                ...
                .catch(error => {
                    ...
                    Swal.fire({
                        icon: 'error',
                        title: 'Error!',
                        html: `<div>An error occurred while updating the invoice.</div><div class="text-muted small mt-2">${error.message}</div>`,
                        confirmButtonColor: '#3085d6',
                    });
                });
        }
            ...
            <div class="card invoice-preview-card p-sm-12 p-6">
                    ...
                    <div class="d-flex flex-wrap flex-column flex-sm-row justify-content-between text-heading">
                        <div class="mb-md-0 mb-6">
                            ...
                            @if ($company && $branch)
                                ...
                                <p class="mb-2">
                                    {{ $company->city ?? '' }}@if ($company->city && ($company->state || $company->zip))
                                        ,
                                    @endif{{ $company->state ?? '' }} {{ $company->zip ?? '' }}
                                </p>
                            ...
                            <dl class="row mb-0 gx-4">
                                ...
                                <dd class="col-sm-7">
                                    <input type="text" class="form-control" disabled value="{{ $invoiceNumber }}"
                                        id="invoiceId" name="invoice_number" />
                                </dd>
                ...
                <div class="card-body px-0">
                    <div class="row">
                        <div class="col-md-6 col-sm-5 col-12 mb-sm-0 mb-6">
                            <h6>Invoice To:</h6>
                            <select class="form-select mb-4" id="clientSelect" name="client_id_select">
                                <option value=""></option>
                                @if (count($clients) > 0)
                            ...
                            <div id="clientDetails">
                                <p class="mb-1 text-nowrap text-truncate" id="clientCompany">Select a customer to view
                                    details</p>
                ...
                <div class="card-body pt-0 px-0">
                    <form class="source-item" id="invoiceForm" method="POST"
                        action="{{ route('billing.invoices.update', $invoice->id) }}">
                        @csrf
                                                    ...
                                                    <div class="text-heading mt-2">
                                                        ...
                                                        <div class="mb-1">
                                                            ...
                                                            <span class="tax-1 me-2" data-bs-toggle="tooltip"
                                                                data-bs-placement="top" title="Item Tax Rate">
                                                                @if ($invoiceItem->tax_id)
                                                                    {{ optional($invoiceItem->tax)->percentage ?? 0 }}%
                                                                @else
                                                                    0%
                                                                @endif
                                                            </span>
                                                            <input type="hidden" class="item-tax-id"
                                                            ...
                                                            <div class="col-12">
                                                                ...
                                                                <select name="tax-1-input"
                                                                    ...
                                                                    @if (count($taxes) > 0)
                                                                        ...
                                                                        @endforeach
                                                                    @else
                                                                        <option value="0" data-percentage="0">No tax
                                                                            (0%)</option>
                                                                    @endif
                                                                </select>
                                                            </div>
                        ...
                        <div class="col-md-6 d-flex justify-content-end">
                            <div class="invoice-calculations">
                                <div class="d-flex justify-content-between mb-2">
                                    <span class="w-px-100 text-nowrap">Subtotal:</span>
                                    <span class="fw-medium text-heading" id="subtotal-display">$0.00</span>
                                </div>
                                <hr class="my-2" />
                                <div class="d-flex justify-content-between mb-2">
                                    <span class="w-px-100 text-danger text-nowrap">Item Level Discount:</span>
                                    <span class="fw-medium text-heading text-danger"
                                        id="item-level-discount-total-display">-$0.00</span>
                                </div>
                                <div class="d-flex justify-content-between mb-2">
                                    <span class="w-px-100 text-primary text-nowrap">Item Level Tax:</span>
                                    <span class="fw-medium text-heading text-primary"
                                        id="item-level-tax-total-display">+$0.00</span>
                                </div>
                                <hr class="my-2" />
                                <div class="d-flex justify-content-between mb-2">
                                    <span class="w-px-100 fw-medium text-nowrap">Document Net Amount:</span>
                                ...
                                <div class="d-flex justify-content-between mb-2">
                                    <span class="w-px-100 text-nowrap">Document Taxable Amount:</span>
                                ...
                                <div class="d-flex justify-content-between">
                                    <span class="w-px-100 fw-bold text-nowrap">Total:</span>
                ...
                <div class="card-body">
                    <button class="btn btn-primary d-grid w-100 mb-4" data-bs-toggle="offcanvas"
                        ...
                        <span class="d-flex align-items-center justify-content-center text-nowrap">
                            <i class="icon-base bx bx-paper-plane icon-xs me-2"></i>Send Invoice
                        </span>
                    </button>
