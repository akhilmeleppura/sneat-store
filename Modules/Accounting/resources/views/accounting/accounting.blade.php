      ...
      <div class="tab-pane fade show active" id="chart_of_accounts" role="tabpanel">
                    ...
                    @foreach($accounts as $account)
                      <tr>
                        ...
                        <td class="text-center">
                          <button class="btn btn-sm {{ $account->status ? 'btn-danger' : 'btn-success' }}">
        ...
        <div class="modal fade" id="addAccountModal" tabindex="-1" aria-labelledby="addAccountModalLabel" aria-hidden="true">
          <div class="modal-dialog">
            <form method="POST" action="{{ route('accounting.chart.store') }}">
              @csrf
                  ...
                  <div class="mb-3">
                    ...
                    <select name="subcategory_id" id="subcategory_id" class="form-select" required>
                      ...
                      @foreach($subcategories as $sub)
                        <option value="{{ $sub->id }}">{{ $sub->name }} - {{ $sub->text }}</option>
                      @endforeach
                    </select>
                  </div>
